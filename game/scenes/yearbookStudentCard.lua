...
function yearbookStudentCard.draw()
  ...
  love.graphics.printf(player.name, nameArea.x, nameArea.y, nameArea.width)
  love.graphics.setColor(1, 1, 1)
  -- COGNOME
  local surnameArea = screenManager:calcAreaSizes({ xPerc = 0.55, yPerc =  0.44, widthPerc = 0.8, heightPerc = 0.1 })
  love.graphics.setColor(0, 0, 0)
  love.graphics.printf(player.surname, surnameArea.x, surnameArea.y, surnameArea.width)
  love.graphics.setColor(1, 1, 1)
  -- DESCRIZIONE
  local descriptionArea = screenManager:calcAreaSizes({ xPerc = 0.55, yPerc = 0.6, widthPerc = 0.8, heightPerc = 0.1 })
  love.graphics.setColor(0, 0, 0)
  love.graphics.printf(player.description, descriptionArea.x, descriptionArea.y, descriptionArea.width)
  love.graphics.setColor(1, 1, 1)
  -- LISTA ATTIVITA'
  for index, activity in ipairs(player.activities) do
